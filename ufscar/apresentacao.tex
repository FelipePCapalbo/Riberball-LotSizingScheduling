...
\begin{frame}{Todas as parcelas do objetivo estão em reais}
  ...
  \restricoes{%
    (A) balanço de estoque \hfill (B) meta de cobertura \hfill
    (C) horas da máquina \hfill (D) lote mínimo e ligação}

  {\footnotesize
  \begin{align*}
    I_{i,t-1} + \sum_{j \in J_i} x_{ijt}
      &= I_{it} + \bigl(o_{it} + a_{i,t-1} - a_{it}\bigr) + \bigl(\hat{d}_{it} - l_{it}\bigr) \tag{A} \\
    I_{it} + g_{it} &\ge \sigma_{it} \tag{B} \\
    \sum_{i \in I} \Bigl( \tfrac{x_{ijt}}{p_{ij}} + \tau_i\, y_{ijt} \Bigr) &\le \kappa_{jt} \tag{C} \\
    q_i\, y_{ijt} \le x_{ijt} &\le M_{ijt}\, y_{ijt} \tag{D}
  \end{align*}}
  ...
  \notacao{%
    ...
    \sy{a}{atraso} · \sy{l}{venda perdida} · \sy{g}{folga} · \sy{o}{carteira} · 
    \sy{\hat{d}}{previsão líquida} · \sy{\sigma}{meta de cobertura} · \sy{\kappa}{horas} · 
    \sy{p}{taxa} · \sy{\tau}{horas de \textit{setup}} · \sy{q}{lote mínimo} · \sy{m}{margem} · 
    \sy{v}{valor} · \sy{c}{custo horário} · \sy{\beta,\rho}{pesos} · \sy{\theta}{carregamento}}
